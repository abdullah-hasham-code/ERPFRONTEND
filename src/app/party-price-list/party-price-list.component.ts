...
@Component({
  selector: 'app-party-price-list',
  templateUrl: './party-price-list.component.html',
  styleUrls: ['./party-price-list.component.css']
})
export class PartyPriceListComponent {
  constructor(private router: Router, private api: ApiService, private messageService: MessageService, private confirmationService: ConfirmationService) { }
	...
	addUser() {
		this.router.navigate(['party-price-form']);
	}
	cancel() {
	...
	editUser(id: any) {
		this.router.navigate(['party-price-form/' + id]);
	}
	deleteUser(user: any) {
